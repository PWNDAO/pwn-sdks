...
Example of a component for repaying loans:

```tsx
// components/RepayLoan.tsx
'use client';

import React, { useState } from "react";
import { useAccount, useConfig } from "wagmi";
import { SimpleLoanContract, repayLoan } from "@pwndao/v1-core";
import { useRepayLoan } from "@pwndao/sdk-v1-react";
...
export default function RepayLoan() {
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white rounded-lg shadow-md border border-gray-200">
      <div className="p-6">
        <h2 className="text-2xl font-semibold text-center mb-4">Repay Loan</h2>
        ...
        {isSuccess && txHash && (
          <div className="mt-4 p-3 bg-green-100 text-green-700 rounded-md">
            ...
            <a
              href={`${BLOCK_EXPLORERS[chainId as keyof typeof BLOCK_EXPLORERS]}${txHash}`}
              target="_blank"
              rel="noopener noreferrer"
...
You can also implement repayment directly using the core functions:
...
export function useDirectRepayLoan() {
  const config = useConfig();
  
  const repayLoanDirectly = useCallback(async (
    loanId: bigint,
    repayer: string,
    chainId: number
  ) => {
  ...
  return { repayLoanDirectly };
}
```

## 5. Custom Authentication with useUserNonces
